...
__tile_global__ void conv2d_implicit_gemm_kernel(const float* input, const float* weights,
    const float* conv_bias, const float* model_bias, float* output) {
    convolution_body<false, true, true, true, N, CI, CO, 1, H, W, 1, OH, OW,
...
__tile_global__ void conv2d_mma_kernel(const T* input, const T* weights,
    const T* conv_bias, const T* model_bias, T* output) {
    convolution_mma_body<T, false, true, true, true, N, CI, CO, 1, H, W, 1, OH, OW,
